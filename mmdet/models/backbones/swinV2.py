...
class SwinTransformerV2(BaseModule):
    ...
    def forward(self, x):
        x, hw_shape, markers = self.patch_embed(x)

        if self.use_abs_pos_embed:
            markers = torch.tensor(markers, device=x.device).repeat(x.shape[0], x.shape[1], 1).transpose(1, 2)
            self.absolute_pos_embed = nn.Parameter(markers)
